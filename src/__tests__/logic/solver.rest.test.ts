...
export const restTests = () => {
  describe('Rest版ソルバー', () => {
    ...
    it('新しい休憩ステージを解く', () => {
      // URL: http://localhost:5173/stage?cells=h4w5grwwwcbbbbbbbbbbswwwg&panels=h2w1gbb&mode=play
      const stageData = 'cells=h4w5grwwwcbbbbbbbbbbswwwg&panels=h2w1gbb'
      const { cells, panels } = decodeStageFromUrl(stageData)
      
      const solutions = solveAllWithRest(cells, panels)
      
      console.log(`新しい休憩ステージ解の数: ${solutions.length}`)
      solutions.forEach((solution, i) => {
        console.log(`解${i}:`)
        solution.phases.forEach((phase, phaseIndex) => {
          console.log(`  フェーズ${phaseIndex}:`)
      ...
      expect(solutions.length).toBeGreaterThan(0)
      expect(solutions[0].phases.length).toBeGreaterThan(1) // 複数フェーズであることを確認
    })
  })
}