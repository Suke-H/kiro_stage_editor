...
def is_passable(cell) -> bool:
    """
    C# の `CanBePassedByPlayer` と同じルール
...
def bfs_shortest_path(grid: Grid) -> list[tuple[int, int]]:
    """
...
async def judge_check(grid: Grid):
    ...
    """
    path_xy = bfs_shortest_path(grid)

    if not path_xy:                   # 経路が無い
        result_enum = Result.NoPath
        path_model = Path(root=[])    # 空経路
    else:
        result_enum = Result.HasClearPath
        vectors = [Vector(x=x, y=y) for x, y in path_xy]
        print(vectors)
        path_model = Path(root=vectors)

    response = PathResult(path=path_model, result=result_enum)
    return {"status": "ok", "data": jsonable_encoder(response)}
